...
`thaimonth.db` backups (`thaimonth.db.bak-*`) are created ad hoc before destructive scripts (`scripts/migrate.py`, `scripts/rescrape_details.py`) — follow that convention (`cp thaimonth.db thaimonth.db.bak-$(date +%Y%m%d-%H%M%S)`) before running scripts that bulk-mutate the database.
